...
router.post('/share', (req, res) => {
    (async () => {
        try {
            ...
            const finalPath = path.join(getFilesDir(), storedFileName);
            ...
            const entry = {
                ...
                ratings: [],
                deleted: false,
                avatar: storedFileName,   // filename to serve via /avatar/:id
                ext,
            };
...
router.get('/avatar/:id', (req, res) => {
    ...
    if (!entry || !entry.avatar) return res.sendStatus(404);

    const filePath = path.resolve(getFilesDir(), entry.avatar);
    if (!fs.existsSync(filePath)) return res.sendStatus(404);

    const ext = (entry.ext || '').toLowerCase();
    const contentTypeMap = /** @type {Record<string,string>} */ ({ png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', gif: 'image/gif', webp: 'image/webp' });
...
router.post('/import/:id', (req, res) => {
    (async () => {
        try {
            ...
            const filePath = path.resolve(getFilesDir(), entry.avatar || '');
            if (!entry.avatar || !fs.existsSync(filePath)) {
